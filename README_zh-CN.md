...
聊天界面支持使用 `#` 触发器的 "提示词 DSL" 功能。这允许用户快速插入预定义的提示词模板或上下文修饰符。

- **触发方式**: 在输入框中输入 `#` 即可查看可用触发器列表。
- **胶囊 UI**: 选中的触发器将以可视化的 "胶囊" 或 "标签" 形式显示在输入区域上方，保持输入文本整洁。
- **执行逻辑**: 发送消息时，与活动标签关联的内容会自动添加到用户消息的前面。
